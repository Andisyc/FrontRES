...
\subsection{General Motion Tracking for Humanoids}
	...
	% PHC, ExBody2, OmniH2O, HumanPlus, Any2Track, KungfuBot2, RobotDancing, AMO, HOVER, GMT, UniTracker, SONIC
	...
	% HumanPlus 构建了全栈模仿管线：WHAM 从单 RGB 相机实时估计人体姿态，GMR 重定向到机器人，Humanoid Shadowing Transformer 零样本执行。首次实现了"看人→跟做"，但未处理视频姿态估计引入的伪影
	...
	RL-based humanoid motion tracking has advanced rapidly. This section traces the progression: from imitating individual motions, to unified whole-body policies trained on diverse datasets, to recent efforts toward general-purpose tracking at scale. PHC trains an adversarial motion prior (AMP) on large-scale motion capture data to map human joint motion onto physically simulated characters. It was the first to demonstrate real-time whole-body tracking of diverse motions with a single policy. ExBody2 decomposes whole-body control into two parts: the upper body tracks reference joint angles from motion capture, while the lower body independently follows root velocity commands via RL. The legs are not forced to match the reference trajectory - they decide how to maintain balance on their own. OmniH2O uses a distillation policy to map multimodal inputs---VR headsets, motion capture suits, RGB cameras---into a shared control space. A single policy handles both real-time teleoperation and offline autonomous execution. HumanPlus builds a full-stack imitation pipeline: WHAM estimates human pose from a single RGB camera in real time, GMR retargets the pose to the robot, and the Humanoid Shadowing Transformer executes zero-shot. It was the first to demonstrate 'see-to-do' end-to-end, but does not address artifacts introduced by video pose estimation. Any2Track consists of two stages: AnyTracker trains a generalist tracker via canonicalized action spaces and expert distillation; AnyAdapter extracts dynamics embeddings from interaction history through a world model and applies online adaptation on top of the frozen policy, keeping the robot stable under combined disturbances such as kicks, rope pulling, and uneven terrain. KungfuBot2 uses a staged curriculum to progressively expand policy capability---training from static poses to highly dynamic martial arts motions step by step, achieving minute-scale continuous execution of kicks, spins, and jumps. RobotDancing's policy does not predict absolute joint commands; it outputs residual corrections $\Delta q$ on top of reference joint targets. It uses distribution-aware and failure-aware sampling during training to ensure sufficient coverage of long-tail, high-dynamic motions (jumps, spins, cartwheels), achieving zero-shot sim-to-real transfer of multi-minute continuous dance. AMO dynamically adjusts per-body-part tracking weights and physics constraint strengths during training, letting the policy learn to balance precise tracking against physical feasibility, rather than relying on hand-tuned fixed reward weights. HOVER uses a single Transformer policy to simultaneously support multiple control modes---joint positions, keypoint poses, root velocity commands, and more. All modes share a common motion encoding space, enabling experience to transfer across modes. GMT introduces two techniques: adaptive sampling that rebalances the training distribution based on motion difficulty, and a Motion Mixture-of-Experts architecture that assigns different motion regions to different expert sub-networks. It serves as the frozen tracker in this work. UniTracker employs a teacher-student pipeline: the teacher learns a latent motion representation with privileged information in simulation, and the student distills this into a proprioception-only policy. The implicit motion encoding lets the tracker understand the current motion intent without requiring explicit per-frame reference poses. SONIC explores the path from scaled-up tracking to generalist humanoid control by fusing multi-source motion capture data, increasing model capacity, and supporting diverse motion input interfaces. It is the largest-scale open-source whole-body tracker to date.

	AMP~\cite{amp2021}
	
	PHC~\cite{phcluo2023}
	ExBody2~\cite{exbodyji2024}
	OmniH2O~\cite{omnih2ohe2024}
	HumanPlus~\cite{humanplus2024}
	Any2Track~\cite{zhang2025track}
	KungfuBot2~\cite{han2025kungfubot2}
	RobotDancing~\cite{sun2025robotdancing}
	AMO~\cite{li2025amo}
	HOVER~\cite{hover2025}
	GMT~\cite{gmtchen2025}
	UniTracker~\cite{uniyin2025}
	SONIC~\cite{sonic2025}
	...
	These works demonstrate that general motion trackers are no longer the bottleneck for motion diversity. Yet they share a common premise: the reference motion itself is dynamically executable. When the reference comes from in-the-wild videos and already contains spatial artifacts before reaching the tracker, the tracker's own robustness is insufficient to compensate for the upstream signal degradation. This paper starts from exactly this observation---that the premise does not always hold.
...
\subsection{From In-the-Wild Videos to Reference Motions}
	...
	% HMR, GLAMR, TRACE, WHAM, GVHMR, TRAM, PHMR
	...
	As trackers improve, attention shifts upstream to the source of reference motions. Recovering 3D human pose and global trajectory from monocular video is the key to scalable reference acquisition. This section reviews the progression: from single-frame mesh recovery to methods that estimate full temporal motion in world coordinates. HMR uses adversarial learning to regress SMPL pose and shape parameters directly from a monocular RGB image, combining 2D keypoint reprojection loss with a 3D adversarial prior, without paired 2D-3D supervision. It is the foundational work in monocular human reconstruction. Its depth ambiguity and pose errors propagate through the entire downstream pipeline---this is the most upstream source of artifacts. GLAMR jointly optimizes camera motion, human pose, and global trajectory under dynamic cameras, using a motion prior to fill in trajectories during occlusions and prevent identity switches or trajectory breaks. TRACE recovers and tracks multiple 3D human trajectories in world coordinates from dynamic camera videos within a single end-to-end network. Its World Motion Map transforms human motion from camera coordinates to global coordinates, unifying multi-person tracking and global trajectory recovery in one stage for the first time. WHAM fuses an AMASS motion prior with SLAM-estimated camera angular velocity. A motion encoder lifts 2D keypoint sequences to 3D representations, which are then fused with video image features to recover the human in global world coordinates. It was the first video-based method to surpass all single-frame methods in per-frame accuracy. Its global trajectory fidelity is bounded by SLAM - camera motion estimates become unreliable under dynamic human occlusion. GVHMR replaces absolute world coordinates with gravity-view coordinates: it fixes the gravity direction as a reference axis and only estimates rotation and translation relative to gravity. This significantly reduces root orientation drift under tilted cameras and sloped terrain. However, the gravity direction must itself be estimated from video, and errors remain unavoidable during fast motion or when no horizon reference is visible. TRAM recovers global human trajectory and motion in two steps: it first robustifies SLAM to recover a metric-scale camera trajectory from static backgrounds, then uses a video Transformer (VIMO) to regress body motion. It masks out dynamic human regions so SLAM relies only on the static scene. However, SLAM can still diverge when background texture is sparse or the scene is highly dynamic. PHMR injects text or 2D visual prompts as conditioning signals into the reconstruction network, using CLIP embeddings to align textual descriptions with human pose features, so that the output is not only accurate but also semantically consistent.

	HMR~\cite{hmr2018}
	GLAMR~\cite{yuan2022glamr}
	TRACE~\cite{sun2023trace}
	WHAM~\cite{whamshin2024}
	GVHMR~\cite{shen2024world}
	TRAM~\cite{tramwang2024}
	PHMR~\cite{wang2025prompthmr}
	...
	The 3D human motions produced by these methods are generally plausible in pose space. But the inherent ambiguity of monocular reconstruction means that spatial errors inevitably remain in the recovered global trajectory: root height drift, absolute orientation offset, accumulated horizontal displacement. These errors are visually subtle but sufficient to alter the physical conditions the tracker depends on. Existing correction methods rely on offline optimization and full sequence access---they need all future frames and substantial solve time. When the goal is zero-shot immediate execution, offline correction is inherently inapplicable. The current pipeline, between kinematic reconstruction and the tracker, lacks an online, per-frame, sequence-independent front-end correction stage.

	% PHC 将 SMPL 人体姿态通过关键点匹配映射到人形机器人关节空间，是 RL-based tracking 中早期常用的重定向方案
	PHC maps SMPL human poses to humanoid joint space via keypoint matching. It was the earlier and widely used retargeting scheme in RL-based tracking pipelines.

	% GMR 使用两阶段 IK 优化实现人体到机器人的运动重定向：先优化根节点朝向消除初始位姿偏差，再做全身微调。其关键创新是非均匀局部缩放——对不同身体部位使用独立的缩放因子
	GMR uses two-stage IK optimization for human-to-humanoid motion retargeting: it first optimizes root orientation to eliminate initial pose bias, then performs full-body fine-tuning. Its key innovation is non-uniform local scaling---applying independent scaling factors to different body parts rather than a single global factor.

	% OmniRetarget 在机器人、物体、地面之间构建交互网格，通过拉普拉斯形变能量在重定向过程中保持空间交互关系不变，同时施加硬物理约束（关节限位、自碰撞、足底接触）。它能保证输出轨迹物理自洽，但无法修正输入人体姿态中已存在的空间误差
	OmniRetarget constructs an interaction mesh among robot, objects, and ground, using Laplacian deformation energy to preserve spatial interaction relationships during retargeting while imposing hard physics constraints (joint limits, self-collision, foot contact). It guarantees physically self-consistent output trajectories, but cannot correct spatial errors already present in the input human pose.

	% ProtoMotions 从动捕数据提取有限个原型姿态作为动作基，用对抗运动先验将真实人体动作投影到原型组合上。在重定向管线中作为可选后处理步骤，对违反物理约束的关节轨迹做平滑修正
	ProtoMotions extracts a finite set of prototype poses from motion capture data as an action basis, then projects real human motions onto combinations of these prototypes via an adversarial motion prior. It serves as an optional post-processing step in the retargeting pipeline, smoothing joint trajectories that violate physics constraints.

	PHC~\cite{phcluo2023}
	GMR~\cite{gmr2025}
	OmniRetarget~\cite{yang2025omniretarget}
	ProtoMotions~\cite{ProtoMotions}
...
\subsection{Residual Learning and Sim-to-Real Adaptation}
	...
	% RMA, ASAP, ResMimic, MOSAIC, OmniXtreme

	% Residual RL 率先提出残差强化学习范式：冻结手写反馈控制器，在其上训练 RL 策略输出加性残差动作。基座策略负责已知动力学，残差策略仅学习接触密集和不确定性区域。FEMR 沿用同一架构思想——残差加在参考空间而非动作空间
	...
	Residual learning offers a correction paradigm that avoids retraining the backbone: freeze an existing base controller and train a lightweight additive correction module on top. This section traces the paradigm from robotic manipulation to legged locomotion to recent humanoid whole-body control. Residual RL first formalized the residual reinforcement learning paradigm: freeze a hand-engineered feedback controller and train an RL policy to output additive residual actions. The base handles known dynamics; the residual only learns contact-rich and uncertain regions. FEMR adopts the same architectural idea - with residuals applied in reference space rather than action space. RMA decomposes the policy into a base and an adaptation module: the base is trained in diverse simulation (varying friction, terrain, payloads) and then frozen; the adaptation module infers environment latent variables from proprioceptive history in real time via supervised learning and modulates the base accordingly. It was the first to demonstrate online multi-terrain adaptation on a real legged robot. ASAP first trains a motion-tracking policy, then trains a delta action model on real-world data to compensate for simulation dynamics mismatch, and finally distills the delta model back into the base policy. The frozen delta model serves as a dynamics correction layer for the simulator; the base policy is fine-tuned in this augmented simulation and deploys without the delta model. It achieves zero-shot transfer of agile motions such as jumping and backflips. ResMimic trains in two stages: it first pretrains a generalist tracking policy as a motion prior, then freezes that prior and trains a residual policy for specific manipulation tasks. The residual only learns manipulation-related joint corrections, leaving the already-learned motion skills undisturbed. The residual is applied in action space. MOSAIC freezes a generalist tracking policy and trains a lightweight residual module on a small amount of teleoperation data, using dual-teacher distillation to inject the residual correction into the tracker while preventing catastrophic forgetting. Its architecture is the closest to FEMR - frozen tracker plus lightweight correction - but the correction is applied in action space. OmniXtreme uses flow matching to generate a motion prior distribution from large-scale motion capture data, then applies residual RL post-training to bridge the sim-to-real gap. The structured motion prior from a generative model replaces the traditional trajectory-tracking reference signal, allowing the policy to explore in behavior space rather than trajectory space. The residual still acts on the tracker output.

	Residual RL~\cite{johannink2019residual}
	RMA~\cite{kumar2021rma}
	ASAP~\cite{asap2025}
	ResMimic~\cite{resmimic2025}
	MOSAIC~\cite{mosaic2026}
	OmniXtreme~\cite{omnixtreme2026}
	...
	Residual learning has been validated across diverse robotic tasks. In humanoid motion control, however, existing residual methods all apply corrections in action space---they modify the tracker's output. When the problem originates in the upstream reference signal rather than the tracker itself, applying corrections directly in reference space is more direct. Online, per-frame, front-end residual correction at the reference level---this direction remains unexplored.
